...
export default function UserProfileSection() {
    ...
    const statusMap = {
        0: {
            text: t("statusMap.unverified"),
            color: "red",
            description: t("statusMap.unverifiedDesc"),
        },
        1: {
            text: t("statusMap.verified"),
            color: "green",
            description: t("statusMap.verifiedDesc"),
        },
        2: {
            text: t("statusMap.pending"),
            color: "yellow",
            description: t("statusMap.pendingDesc"),
        },
        3: {
            text: t("statusMap.rejected"),
            color: "red",
            description: t("statusMap.rejectedDesc"),
        },
    };
    ...
    const fetchResellerInfo = useCallback(async () => {
        try {
            ...
            if (localValue === null) {
                const serverEnabled =
                    response.data.data.reseller_switch === "1";
    ...
    const handleSwitchReseller = async () => {
        ...
        try {
            ...
            if (response.data && response.data.message.success) {
                ...
                toast.success(
                    t("messages.switchSuccess", {
                        status: newStatus ? t("enabled") : t("disabled"),
                    }),
                );
            } else if (response.data?.message?.error) {
                toast.error(
                    response.data.message.error[0] || t("errors.switchFailed"),
                );
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        const errors = [];
        if (!frontFile) errors.push(t("errors.frontRequired"));
        if (!backFile) errors.push(t("errors.backRequired"));
        ...
        const toastId = toast.loading(t("messages.submitting"));

        try {
            const response = await resellerSubmitAPI(
                idType,
                frontFile,
                backFile,
                lang
            ...
            if (err.response?.data?.message?.error) {
                ...
                toast.error(t("errors.fixErrors"), { id: toastId });
            } else {
    ...
    const {
        ...
        onDrop: (acceptedFiles) => {
            if (acceptedFiles.length > 0) {
                ...
                setResellerErrors(
                    resellerErrors.filter(
                        (e) => !e.includes("front") && !e.includes("Front"),
                    ),
                );
            }
        },
                ...
                .map(({ file, errors }) =>
                    errors.map((error) => {
                        if (error.code === "file-too-large") {
                            return t("errors.fileTooLarge", { side: "Front" });
                        }
    ...
    const {
        ...
        onDrop: (acceptedFiles) => {
            if (acceptedFiles.length > 0) {
                ...
                setResellerErrors(
                    resellerErrors.filter(
                        (e) => !e.includes("back") && !e.includes("Back"),
                    ),
                );
            }
        },
        ...
        onDropRejected: (fileRejections) => {
            const newErrors = fileRejections
                .map(({ file, errors }) =>
                    errors.map((error) => {
                        if (error.code === "file-too-large") {
                            return t("errors.fileTooLarge", { side: "Back" });
                        }
                        if (error.code === "file-invalid-type") {
                            return t("errors.invalidFileType", {
                                side: "Back",
                            ...
                            {submittedData && (
                                <div className="border border-gray-200 rounded-md p-5">
                                    ...
                                    <div className="space-y-4">
                                        {submittedData.submitted_Data.map(
                                            (field, index) => (
                                                <div
                                                    key={index}
                                                        ...
                                                        <div className="flex items-center gap-3 mt-2">
                                                            <Image
                                                                ...
                                                                onError={(
                                                                    ...
                                                                    e.target.src =
                                                                        submittedData.default_image_path;
                                                                }}
                                                            />
                            ...
                            <div>
                                ...
                                <Listbox value={idType} onChange={setIdType}>
                                    <div className="relative mt-1">
                                        <Listbox.Button className="relative w-full cursor-pointer rounded-md border border-gray-300 bg-white py-2 pl-3 ltr:pr-10 rtl:pr-5 ltr:text-left rtl:text-right shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                            <span className="block truncate">
                                                {idType || t("selectIdType")}
                                            </span>
                                            <span className="pointer-events-none absolute inset-y-0 ltr:right-0 rtl:left-0 flex items-center pr-2 pl-2">
                                        ...
                                        <div
                                            ...
                                            className={`border-2 border-dashed rounded-md p-4 text-center cursor-pointer transition-colors duration-200 flex-1 ${
                                                isFrontDragActive
                                                    ...
                                                    : "border-gray-300"
                                            } ${resellerErrors.some((e) => e.includes("front") || e.includes("Front")) ? "border-red-500 bg-red-50" : ""}`}
                                        >
                                        ...
                                        <div
                                            ...
                                            className={`border-2 border-dashed rounded-md p-4 text-center cursor-pointer transition-colors duration-200 flex-1 ${
                                                isBackDragActive
                                                    ? "border-blue-500 bg-blue-50"
                                                    : "border-gray-300"
                                            } ${resellerErrors.some((e) => e.includes("back") || e.includes("Back")) ? "border-red-500 bg-red-50" : ""}`}
                                        >
