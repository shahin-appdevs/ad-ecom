...
export default function MakePaymentSection({ setRefetch }) {
    ...
    useEffect(() => {
        (async () => {
            if (
                !makePaymentData?.get_remaining_fields ||
                !makePaymentData?.makePaymentCharge ||
                !selectedCurrency
            ) {
                return;
            }
            try {
                ...
                const chargeId = makePaymentData?.makePaymentCharge?.id;
    ...
    }, [amount, makePaymentData, selectedCurrency]);
    ...
    const limitText = useMemo(() => {
        if (selectedCurrency) {
            const minLimit =
                parseFloat(makePaymentData.makePaymentCharge.min_limit || 0) *
                parseFloat(selectedCurrency.rate || 0);
            const maxLimit =
                parseFloat(makePaymentData.makePaymentCharge.max_limit || 0) *
                parseFloat(selectedCurrency.rate || 0);
            return `${minLimit.toFixed(4)} - ${maxLimit.toFixed(4)} ${selectedCurrency.code}`;
        }
        return "0.00 - 0.00";
    }, [selectedCurrency, makePaymentData.makePaymentCharge]);
    ...
    const feesCalculation = useMemo(() => {
        ...
        const fixedCharge =
            parseFloat(makePaymentData.makePaymentCharge.fixed_charge) *
            parseFloat(selectedCurrency.rate);
        const percentCharge =
            (amountNum *
                parseFloat(makePaymentData.makePaymentCharge.percent_charge)) /
            100;
    ...
    const limitsCalculation = useMemo(() => {
        ...
        const minLimit = (
            parseFloat(makePaymentData.makePaymentCharge.min_limit) /
            exchangeRate
        ).toFixed(4);
        const maxLimit = (
            parseFloat(makePaymentData.makePaymentCharge.max_limit) /
            exchangeRate
        ).toFixed(4);
        const dailyLimit = (
            parseFloat(makePaymentData.makePaymentCharge.daily_limit) /
            exchangeRate
        ).toFixed(4);
        const monthlyLimit = (
            parseFloat(makePaymentData.makePaymentCharge.monthly_limit) /
            exchangeRate
        ).toFixed(4);
    ...
    const handleMakePayment = async () => {
        try {
            ...
            toast.error(
                error.response?.data?.message?.error?.[0] || t("failedPayment"),
            );
        } finally {
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        ...
                        <div className="bg-[#F9FAFB] border border-gray-200 shadow-sm p-4 rounded-xl text-center space-y-1.5">
                            ...
                            <h6 className="font-medium mt-1" dir="ltr">
                                {makePaymentData.makePaymentCharge.fixed_charge}{" "}
