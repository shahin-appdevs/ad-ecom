...
export default function MoneyOutSection({ setRefetch }) {
    ...
    useEffect(() => {
        (async () => {
            if (
                !moneyOutData?.get_remaining_fields ||
                !moneyOutData?.moneyOutCharge ||
                !selectedCurrency
            ) {
                return;
            }
            try {
                ...
                const getRemainingFields = moneyOutData?.get_remaining_fields;
                ...
                const chargeId = moneyOutData?.moneyOutCharge?.id;
    ...
    }, [amount, moneyOutData, selectedCurrency]);
    ...
    const limitText = useMemo(() => {
        if (selectedCurrency) {
            const minLimit =
                parseFloat(moneyOutData.moneyOutCharge.min_limit || 0) *
                parseFloat(selectedCurrency.rate || 0);
            const maxLimit =
                parseFloat(moneyOutData.moneyOutCharge.max_limit || 0) *
                parseFloat(selectedCurrency.rate || 0);
            return `${minLimit.toFixed(4)} - ${maxLimit.toFixed(4)} ${selectedCurrency.code}`;
        }
        return "0.00 - 0.00";
    }, [selectedCurrency, moneyOutData.moneyOutCharge]);
    ...
    const feesCalculation = useMemo(() => {
        ...
        const fixedCharge =
            parseFloat(moneyOutData.moneyOutCharge.fixed_charge) *
            parseFloat(selectedCurrency.rate);
        const percentCharge =
            (amountNum *
                parseFloat(moneyOutData.moneyOutCharge.percent_charge)) /
            100;
    ...
    const limitsCalculation = useMemo(() => {
        ...
        const minLimit = (
            parseFloat(moneyOutData.moneyOutCharge.min_limit) / exchangeRate
        ).toFixed(4);
        const maxLimit = (
            parseFloat(moneyOutData.moneyOutCharge.max_limit) / exchangeRate
        ).toFixed(4);
        const dailyLimit = (
            parseFloat(moneyOutData.moneyOutCharge.daily_limit) / exchangeRate
        ).toFixed(4);
        const monthlyLimit = (
            parseFloat(moneyOutData.moneyOutCharge.monthly_limit) / exchangeRate
        ).toFixed(4);
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const response = await moneyOutGetAPI(lang);
                const data = response.data.data;
                setMoneyOutData({
                    ...data,
                    moneyOutCharge:
                        data.moneyOutCharge || moneyOutData.moneyOutCharge,
                });
                setIsLoading(false);
            } catch (error) {
    ...
    useEffect(() => {
        const checkUser = async () => {
            if (credentials && credentials.length > 3) {
                try {
                    await moneyOutCheckAgentAPI(credentials, lang);
                } catch (error) {
    ...
    const handleScanSuccess = async (qrCode) => {
        try {
            const response = await moneyOutScanAPI(qrCode, lang);
