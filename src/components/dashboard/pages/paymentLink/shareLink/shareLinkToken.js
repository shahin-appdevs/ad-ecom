...
import { paymentLinkShareAPI, paymentLinkShareSubmitAPI } from "@root/services/apiClient/apiClient";
...
const StripeCardForm = ({ formData, onFormDataChange, publicKey, onCardDetailsChange }) => {
...
function PaymentLinkShareContent({ data, publicKey, formData, setFormData, paymentType, setPaymentType, apiLoading, setApiLoading }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (paymentLink.type === 'pay' && !paymentLink.price && !formData.amount) {
        ...
        try {
            ...
            } else if (paymentType === 'card_payment' || paymentType === 'wallet_payment') {
            ...
            toast.error(error.response?.data?.message?.error?.[0] || 'Payment failed');
        } finally {
            setApiLoading(false);
        }
    };
    ...
    return (
        <div className="bg-white rounded-[12px] p-4 sm:p-6 lg:p-7">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
                ...
                <div>
                    <h3 className="text-xl font-bold mb-2">{paymentLink.title}</h3>
                    {paymentLink.details && <p className="text-gray-600 mb-8">{paymentLink.details}</p>}
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-3">
                        <div>
                            <p className="mb-1"><span className="font-medium">Type:</span> {paymentLink.linkType}</p>
                            <p className="mb-1"><span className="font-medium">Amount:</span> {paymentLink.amountCalculation}</p>
                            <p className="mb-1"><span className="font-medium">Currency:</span> {paymentLink.currency} ({paymentLink.currency_symbol})</p>
                            {paymentLink.limit && <p className="mb-1"><span className="font-medium">Limit:</span> {paymentLink.limit} payments</p>}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {paymentLink.type === 'pay' && !paymentLink.price && (
                            <div>
                                <label className="block text-sm font-medium mb-1">Amount ({paymentLink.currency_symbol})</label>
                                <input
                                    type="number"
                                    name="amount"
                                ...
                                {paymentLink.min_amount && (
                                    <p className="text-xs text-gray-500 mt-1">Minimum: {paymentLink.min_amount} {paymentLink.currency}</p>
                                )}
                                {paymentLink.max_amount && (
                                    <p className="text-xs text-gray-500 mt-1">Maximum: {paymentLink.max_amount} {paymentLink.currency}</p>
                                )}
                            </div>
                        )}
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                <Listbox value={paymentType} onChange={setPaymentType}>
                                    <Listbox.Label className="block text-sm font-medium">Payment Method *</Listbox.Label>
                                    <div className="relative">
                                        <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left border focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                                            <span className="block truncate">
                                                {paymentType === 'payment_gateway' && 'Payment Gateway'}
                                                {paymentType === 'card_payment' && 'Card'}
                                                {paymentType === 'wallet_payment' && 'Wallet'}
                                            </span>
                                        ...
                                        <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-10">
                                            {data.payment_settings.payment_gateway_status && (
                                                <Listbox.Option
                                                    key="payment_gateway"
                                            ...
                                            {data.payment_settings.card_status && (
                                                <Listbox.Option
                                                    key="card_payment"
                                                    value="card_payment"
                                                    className={({ active }) =>
                                            ...
                                            {data.payment_settings.wallet_status && (
                                                <Listbox.Option
                                                    key="wallet_payment"
                            ...
                            {paymentType === 'payment_gateway' && (
                                <div>
                                    <Listbox value={formData.paymentGateway} onChange={(value) => handleInputChange({ target: { name: 'paymentGateway', value } })}>
                                        <Listbox.Label className="block text-sm font-medium mb-1">Select Gateway *</Listbox.Label>
                                        <div className="relative mt-1">
                                            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left border focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                                                <span className="block truncate">
                                                    {formData.paymentGateway 
                                                        ? data.payment_gateways.find(g => g.alias === formData.paymentGateway)?.name 
                                                        : 'Select a gateway'}
                                                </span>
                                                <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                                                    <ChevronUpDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
                                            ...
                                            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-10">
                                                ...
                                                {data.payment_gateways.map((gateway) => (
                                                    <Listbox.Option
                                                        key={gateway.id}
                                                        value={gateway.alias}
                                                        className={({ active }) =>
                            ...
                            {paymentType === 'wallet_payment' && (
                                <div>
                                    <Listbox value={formData.walletCurrency} onChange={(value) => handleInputChange({ target: { name: 'walletCurrency', value } })}>
                                        <Listbox.Label className="block text-sm font-medium mb-1">Select Wallet Currency *</Listbox.Label>
                                        <div className="relative mt-1">
                                            <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left border focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                                                <span className="block truncate">
                                                    {formData.walletCurrency 
                                            ...
                                            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-10">
                                                ...
                                                {data.active_currencies.map((currency) => (
                                                    <Listbox.Option
                                                        key={currency.id}
                                                        value={currency.code}
                                                        className={({ active }) =>
