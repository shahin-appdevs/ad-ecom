...
import { productGetSellerAPI, UpdateProductSellerAPI, editProductSellerAPI } from "@root/services/apiClient/apiClient";
...
export default function ProductEditSection() {
    ...
    const fetchProductData = async (productId) => {
        ...
        const response = await editProductSellerAPI(productId);
            ...
            toast.error(error.response?.data?.message?.error?.[0] || "Failed to load product");
        } finally {
            setLoading(false);
        }
    };
