...
export default function PointConversionSection() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setTransactions(response.data.data.transactions.point_conversion || []);
            } catch (error) {
    ...
    return (
        <div className="bg-white rounded-[12px] p-7">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between pb-3 mb-2 gap-3 border-b-[1.5px] border-[#F5F7FF]">
                <h2 className="text-[16px] font-semibold">Point Conversion Logs</h2>
            </div>
            ...
            {!apiData ? (
                    ...
                    <table className="min-w-full divide-y divide-[#F5F7FF] whitespace-nowrap">
                        <thead>
                            <tr className="bg-[#F5F7FF] text-left text-sm text-color__paragraph">
                                ...
                                <th className="py-4 px-5 font-semibold">Attribute</th>
                                <th className="py-4 px-5 font-semibold">Request Point</th>
                                <th className="py-4 px-5 font-semibold">Request Amount</th>
                                <th className="py-4 px-5 font-semibold">Total Charges</th>
                                <th className="py-4 px-5 font-semibold">Will Get</th>
                ...
                <div className="text-center py-5">No point conversion transactions found</div>
            ) : (
                <div className="table-wrapper overflow-x-auto">
                    <table className="min-w-full divide-y divide-[#F5F7FF] whitespace-nowrap">
                        <thead>
