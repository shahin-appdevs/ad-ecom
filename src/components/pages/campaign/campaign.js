        ...
        const fetchCampaigns = async () => {
            try {
                ...
                if (response?.data?.data?.all_campaigns?.data) {
                    const publicCampaigns = response.data.data.all_campaigns.data.filter(
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {campaigns.map((campaign) => (
                                <Link href={`/campaigns/products?id=${campaign.id}`} key={campaign.id}>
                                    <div className="w-full sm:h-[175px] relative group">
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent">
                                            ...
                                            <p className="text-white text-xs truncate" dangerouslySetInnerHTML={{ __html: campaign.description}} />
                                        </div>
                                    </div>
                                </Link>
                            ))}
